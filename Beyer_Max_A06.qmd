...
[https://github.com/maxbeyer1/Beyer_Max_201_A06](https://github.com/maxbeyer1/Beyer_Max_201_A06)
...
![](screenshot.png)
(project name is really Beyer_Max_201_A06 as shown in GitHub link but I named the local directory assignment_6 to be consistent, which is what is showing in the screenshot)
...
```{r}
# Extract the value `8162` (LeBron James points):
# indexing 
result_indexing <- player_info[[2]][[4]]
result_indexing

# names
result_names <- player_info$LJ$points
result_names
```
...
for (player in player_info) {
  ...
  player_name <- strsplit(player$name, " ")[[1]]
  player_initials <- paste0(substr(player_name[1], 1, 1), substr(player_name[2], 1, 1))
...
points_per_game_rounded <- sapply(points_per_game_list, round_to_two)
...
mat_rounded <- apply(mat_rand, MARGIN = 1:2, round_to_nearest)
...
```{r}
avg_temp_month <- colMeans(max_temps[, 3:14], na.rm = TRUE)
...
```{r}
co_median_annual <- apply(co_temps[, 3:14], 1, median, na.rm = TRUE)

max_median_annual <- which.max(co_median_annual)
min_median_annual <- which.min(co_median_annual)

cat("The maximum median annual temperature of ", co_median_annual[max_median_annual], " occurred in ", co_temps$year[max_median_annual], " and the minimum median annual temperature of ", co_median_annual[min_median_annual], " occurred in ", co_temps$year[min_median_annual], ".", sep="")
```
